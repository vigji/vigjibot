...
class PooledMarket:
    ...
    question: str
    outcomes: List[str]  # List of outcome names, e.g., ["Yes", "No"] or ["Candidate A", "Candidate B"]
    outcome_probabilities: List[Optional[float]]  # Corresponding probabilities for outcomes
    formatted_outcomes: str  # Single string representation, e.g., "Yes: 60.0%; No: 40.0%"
    url: str  # Direct URL to the market
    published_at: Optional[datetime]  # Publication/creation time of the market (UTC if possible)
    ...
    original_market_type: Optional[str] = None  # Platform-specific type, e.g., "BINARY", "MULTIPLE_CHOICE"
    is_resolved: Optional[bool] = None # True if the market is resolved/closed, False if open, None if unknown
...
class BaseMarket(ABC):
    ...
    def parse_datetime_flexible(cls, dt_str: Optional[str]) -> Optional[datetime]:
        ...
        try:
            ...
            elif '+' in dt_str[10:] or '-' in dt_str[10:]: # Check for +/- in the time part
